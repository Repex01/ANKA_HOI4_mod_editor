...
4. [x] Фаза 4 — пиксельное редактирование provinces.bmp + save bmp/csv.
4b.[x] Фаза 4b — авто-генерация N провинций в области (порт `TestFilling`, numpy).
5. [x] Фаза 5 — adjacencies + supplyareas + валидация + локали + README + снять заглушку.
   Дополнительно: синхронизация `map/strategicregions/` при изменении провинций
   (split наследует регион источника; новая нарисованная провинция получает регион
   большинства соседей). Заглушка снята ещё в Фазе 1. Проверено на живой ванили:
   supply areas — легаси (1 файл), озёрные провинции легально входят в штаты.
6. [x] Полный регресс (map + все прочие редакторы + запуск приложения): смоки фаз
       1–5 (сервисы + GUI), сборка ВСЕХ редакторов в скрытом руте, запуск `main.py`.
