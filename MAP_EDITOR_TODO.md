...
## Порядок работы
1. [ ] Фаза 1 — MapService (read) + справочники + **пул свободных цветов** + read-only
       карта с режимами и выбором.
2. [ ] Фаза 2 — block-backed StateDocument + инспектор штата (постройки авто-парс).
3. [ ] Фаза 3 — инспектор провинции (террейн/тип через definition.csv).
