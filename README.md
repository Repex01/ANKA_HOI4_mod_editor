...
│                           карта: map_service (bmp+csv, numpy), terrain/building/map_refs (справочники),
│                           region_gen (авто-генерация провинций), adjacency_service, map_zones
│                           (supplyareas + strategicregions), map_validation, mapgen_colors (пул цветов);
...
### Карта (`editors/map/`)
Провинции, штаты и границы поверх `map/` + `history/states/`. Слои: дерево штатов ·
canvas-вьюпорт · инспекторы «Провинция»/«Штат» (notebook).
- **Канвас** (`canvas.py`): собственная камера (offset+zoom, Ctrl+колесо с якорем в
  курсоре, пан правой/средней), рендер вьюпорта в фоне (latest-request-wins, очередь →
  `after`), индикатор координат/провинции. Режимы окраски: провинции / террейн /
  владельцы / штаты — LUT `id→rgb` по numpy (`MapService.render_view`, полный кадр
  5632×2048 ≈ 0.14 с; выбор подсвечивается, штат — светлее).
- **MapService**: `provinces.bmp` → `uint32`-коды + плотный индекс (`np.unique`),
  `definition.csv` — построчно (нетронутые строки сохраняются байт-в-байт, CRLF и
  строка провинции 0 как в ваниле); no-op сохранение bmp байт-идентично ванильному
  (24-бит без сжатия). Мутации инкрементальны (bbox-кэши, LUT не перестраивается).
- **Инспектор провинции**: тип land/sea/lake (список террейнов переключается),
  террейн-категория, coastal, континент, площадь/bbox/соседи (по пиксельной
  смежности), «к штату», «Разбить…».
- **Инспектор штата**: владелец/контролёр (пикер стран), категория, manpower,
  local_supplies, impassable, корки-чипы, VP, ресурсы, постройки штата и провинций
  (скоуп авто-парсится из `common/buildings`: `level_cap.province_max` → провинция;
  `only_costal` скрывается для неприбрежных), провинции штата (добавить из списка /
  🎯 кликом по карте с переносом из другого штата), зона снабжения, предпросмотр PDX,
  удаление (мод). Ваниль read-only до «Копировать в мод» (байтовая копия-override).
- **Рисование границ**: кисть (радиус, интерполяция штриха), заливка (scanline flood
  по numpy), пипетка, «➕ провинция» (свободный id + уникальный цвет из
  предгенерированного пула `config/data/free_province_colors.bin`, регенерация —
  `python -m anka.services.mapgen_colors`). После правки границ движковые
  `positions.txt` и т.п. устаревают — редактор напоминает про Nudge.
- **Разбиение провинции** (`services/region_gen.py`, порт TestFilling): K связных
  кластеров растут внутри маски (сиды — жадный max-min), стратегии «органическая» /
  «ровные границы», векторизованный мажоритарный smooth, детерминизм по seed;
  превью в будущих цветах → применить. Новые провинции наследуют definition-поля,
  добавляются в штат (по подтверждению) и в стратегический регион исходной.
- **Стратегические регионы** (`services/map_zones.py`): следят за изменениями
  провинций — при split новые id наследуют регион источника; нарисованная с нуля
  провинция получает регион большинства соседей при завершении штриха.
- **Adjacencies** (`map/adjacencies.csv`): таблица проливов/каналов/impassable —
  добавить/править/удалить, правила из `adjacency_rules.txt`; терминатор `-1;-1;…`
  и хвостовой комментарий сохраняются байт-в-байт.
- **Зоны снабжения** (`map/supplyareas/`, легаси после 1.11): показать/сменить зону
  штата, создать зону, `value`.
- **Валидация** (`services/map_validation.py`): сироты definition↔bmp, дубли
  id/цветов, land без штата, провинция в двух штатах, sea в штате (озёра в ваниле
  легальны!), дубли/чужие VP, постройки выше max / `only_costal` не на побережье /
...
> Эти форматы редактирует «Карта» (см. раздел редактора). Для BMP критичны точная
> битность и **отсутствие сжатия** — Pillow пишет 24-бит BMP без RLE, no-op
> сохранение байт-идентично ванильному файлу. Проверенные на практике отличия от
> вики: диапазонов sea/lake в `default.map` нет (тип берётся из `definition.csv`),
> озёрные провинции входят в ванильные штаты, supply areas после 1.11 — легаси.
